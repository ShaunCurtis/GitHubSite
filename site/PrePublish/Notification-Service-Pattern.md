...
precis: The Blazor Notifcation Service Pattern
date: 2021-03-31
---

# Blazor Notification Service Pattern

I'm always suprised by the hoops and amount of plumbing developers are prepared to do to get components to talk to each other.  almost always to make sure they are all up-to-date and displaying the latest set of information.

For example, an editor window updates a list item, or adds a new one.  The list needs to update and the list count needs to update when the addition or update is saved.  Child components need to make callbacks to parents, who then need to trigger updates to other children.  Sound familiar?

As things evolve the plumbing gets more convoluted and difficult to maintain or simply breaks.

## The Notification Service Pattern
There is a simpler and more maintainable solution.  What I call the Notification Service Pattern.  There are various versions around, but they're just different ways of coding the same basic functionality.

The basic service pattern looks like this:
...
public class DataService
{
    ...
    {
        __Values =  await someservice.GetList();
        NotifyListChanged();
    }
...
And the basic component pattern looks like this:
...
@code {
    ...
    {
        ....
        this.DataService.ListChanged += this.OnListChange;
        ....
    }

    private async void OnListChange(object sender, EventArgs e)
...
To demonstrate the principle, lets look at the problem in a simplistic way.
